...
public class UsuarioDaoPostgreSQL implements UsuarioDao {
	...
	private static final String eliminarUsuarioSQL = "DELETE FROM usuario WHERE id=?";
	private static final String actualizarUsuarioSQL = "UPDATE usuario SET nombre=?, email=?,pass=? WHERE id=?";
	private static final String buscarUsuarioId = "SELECT * FROM usuario WHERE id=?";
	private static final String buscarUsuarios = "SELECT * FROM usuario";

	public  UsuarioDaoPostgreSQL() {
		this.conexion = ConexionPostgreSQL.singlenton();
	}
